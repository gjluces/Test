...
void displayLowestRatings(int productRatings[NUM_CUSTOMERS]) {
	int menor;
	menor = productRatings[0];
	for (int i = 0; i < NUM_CUSTOMERS; i++) {
		if (productRatings[i] < menor) {
			menor = productRatings[i];
		}
	}
	cout << "The lowest rating is " << menor << endl;
}
void displaySortedRatingsAsc(int productRatings[NUM_CUSTOMERS]) {
	for (int i = 0; i < NUM_CUSTOMERS-1; i++) {
		for (int j = 0; j < NUM_CUSTOMERS - i - 1; j++) {
			if (productRatings[j] > productRatings[j + 1]) {
				int temp = productRatings[j];
				productRatings[j] = productRatings[j + 1];
				productRatings[j + 1] = temp;
			}
		}
	}
	displayRatings(productRatings);
...
3, 4, 1 };
	...
	while (true) {
		...
		cout << "6) Display Sorted Ratings Desc" << endl;
		cout << "7) Quit" << endl;
		...
		if (choice == 1) {
			...
			displayLowestRatings(productRatings);
		}
		else if (choice == 5) {
			displaySortedRatingsAsc(productRatings);
		}
		else if (choice == 6) {
			displaySortedRatingsDesc(productRatings);
		}
		else if (choice == 7) {
			break;
		}
